...
`sizeBytes` from the real file's length, `sha256` computed
independently in Go by the manifest tool itself (not passed through
from the PowerShell script's own `.sha256` sidecar-file computation -
see that step's own doc comment in `build-release.ps1` for why it uses
.NET's `SHA256` type directly rather than the `Get-FileHash` cmdlet).
The manifest is generated by a small first-party
