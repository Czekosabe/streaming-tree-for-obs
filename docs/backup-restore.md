...
| Goal contribution dedupe ledger | `0025` `goal_applied_events` | Marginal | No (no event content, only provider event keys) | **No** | Not restored in v1 - no repository method exists to list it today, and the practical risk is narrow (only the exact same already-applied provider event replaying after restore would be double-counted, which the real provider event stream essentially never does); `goals.Current`/`Baseline` themselves are still restored. Revisit if a future stage adds a real list method for another reason. |
...
| Stream setup profiles | `0033` `stream_setup_profiles`, `stream_setup_profile_destinations` | Yes | No (structural exclusion proof: `docs/stream-setup-profiles.md` §9/§12) | Yes | Fresh local `id`; restored after Platforms and Metadata presets so each destination's `platformId` and the profile's own `metadataPresetId` remap through the same real ids those two domains were just given - a reference already missing (deleted) before the backup was taken stays missing, never recreated or rebound |
| Stream session history | `0031` `stream_sessions`, `stream_session_destinations` | No | No (operational timeline only - never chat/donation/engagement content, that table's own migration comment) | **No** | Not restored - Stage 24 operational history, the same "not a place operational session history lives" exclusion §0 already states; a restored install starts with zero sessions, exactly like a fresh install |
| Stream session retention preference | `0032` `stream_session_settings` (singleton) | Yes | No | Yes | Singleton restored verbatim - a PREFERENCE about how long to keep history, never the history itself; kept distinct from the row above on purpose |
