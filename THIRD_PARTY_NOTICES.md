...
| [`golang.org/x/sys`](https://pkg.go.dev/golang.org/x/sys) v0.47.0 | BSD-3-Clause | The Go project's own extended platform-syscall package (already an indirect dependency via other modules above); used directly since Stage 20A for the Windows-only `ShellExecuteW`/`CreateMutexW`/`MessageBoxW` calls behind `internal/runtime/browserlaunch`, `internal/runtime/singleinstance`, and `internal/runtime/nativealert` (each build-tagged `windows`, with a no-op/fallback `!windows` counterpart) |
| [`github.com/shirou/gopsutil/v4`](https://github.com/shirou/gopsutil) v4.26.7 | BSD-3-Clause | Cross-platform CPU/memory/disk usage sampling for the Dashboard's "System resources" card (`internal/sysresources`) - a mature (`psutil`-for-Go, used across the Go ecosystem), narrowly-scoped library; only its `cpu`/`mem`/`disk` subpackages are imported, never its `process`/`net`/`host`/`sensors` ones. Every value it returns stays local: sampled in-process, cached in memory, served over this backend's own local HTTP API, never written to disk and never transmitted to any external service |
| [`golang.org/x/crypto`](https://pkg.go.dev/golang.org/x/crypto) v0.55.0 | BSD-3-Clause | The Go project's own extended cryptography package; used directly since Stage 20D2B for `argon2.IDKey` (`internal/auth/password.go`), the RFC 9106 Argon2id password-verifier hashing behind the remote-management single-administrator login (`docs/remote-management.md` §9.1/§2) |
| [`golang.org/x/term`](https://pkg.go.dev/golang.org/x/term) v0.45.0 | BSD-3-Clause | The Go project's own extended terminal package; used directly since Stage 20D2B for `term.ReadPassword` (`cmd/server/provision.go`), reading a new administrator password from stdin without echoing it back to the terminal for the local-only `-provision-admin-password` flag (`docs/remote-management.md` §9.2) |
