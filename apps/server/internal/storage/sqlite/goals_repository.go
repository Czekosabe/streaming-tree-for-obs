...
const widgetProfileColumns = `id, goal_id, name, enabled, public_slug, title_override,
...
func scanWidgetProfile(scanner interface{ Scan(...any) error }) (goals.WidgetProfile, error) {
	var (
		...
		enabled, showCurrent         int
		showTarget, showPercent      int
		orientation, textAlign, font string
		createdAt, updatedAt         string
	)
	if err := scanner.Scan(
		&p.ID, &p.GoalID, &p.Name, &enabled, &p.PublicSlug, &p.TitleOverride,
		&showCurrent, &showTarget, &showPercent, &orientation, &textAlign, &font,
...
func (r *GoalsRepository) CreateWidgetProfile(ctx context.Context, p goals.WidgetProfile) (goals.WidgetProfile, error) {
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO widget_profiles (id, goal_id, name, enabled, public_slug, title_override,
			...
			created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		p.ID, p.GoalID, p.Name, boolToInt(p.Enabled), p.PublicSlug, p.TitleOverride,
...
func (r *GoalsRepository) GetWidgetProfile(ctx context.Context, id string) (goals.WidgetProfile, bool, error) {
	row := r.db.QueryRowContext(ctx, `SELECT `+widgetProfileColumns+` FROM widget_profiles WHERE id = ?`, id)
...
func (r *GoalsRepository) GetWidgetProfileByPublicSlug(ctx context.Context, slug string) (goals.WidgetProfile, bool, error) {
	row := r.db.QueryRowContext(ctx, `SELECT `+widgetProfileColumns+` FROM widget_profiles WHERE public_slug = ?`, slug)
	p, err := scanWidgetProfile(row)
	if errors.Is(err, sql.ErrNoRows) {
		return goals.WidgetProfile{}, false, nil
	}
...
func (r *GoalsRepository) DeleteWidgetProfile(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx, `DELETE FROM widget_profiles WHERE id = ?`, id)
	if err != nil {
