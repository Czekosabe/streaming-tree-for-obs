...
import {
  ...
  useYouTubeIntegrationConfigQuery,
  useSetYouTubeIntegrationConfigMutation,
...
function IntegrationConfigForm() {
  ...
  const configQuery = useYouTubeIntegrationConfigQuery();
  const setConfig = useSetYouTubeIntegrationConfigMutation();
  ...
  const sourceNote = {
    environment: t('accounts:youtube.integration.clientIdSourceEnvironment'),
    database: t('accounts:youtube.integration.clientIdSourceDatabase'),
    missing: t('accounts:youtube.integration.clientIdSourceMissing'),
  }[config.source];

  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-3">
      <p className="text-[11px] text-ink-faint">{t('accounts:youtube.integration.description')}</p>
      <p className="text-[11px] text-ink-faint">{t('accounts:youtube.integration.clientIdHint')}</p>
      ...
      <FormField label={t('accounts:youtube.integration.clientIdLabel')} hint={sourceNote}>
        {({ inputId, describedBy }) => (
          <TextInput
            id={inputId}
            ...
            placeholder={t('accounts:youtube.integration.clientIdPlaceholder')}
            onChange={(event) => {
      ...
      <p className="text-[11px] text-ink-faint">{t('accounts:youtube.integration.noSecretNote')}</p>
      <p className="text-[11px] text-status-warning">{t('accounts:youtube.integration.testingModeNote')}</p>
      {isEnvironment ? null : configQuery.data.configured === false ? null : (
        <p className="text-[11px] text-ink-faint">{t('accounts:youtube.integration.lockedNote')}</p>
      )}
      ...
      {!isEnvironment && (
        <div className="flex items-center gap-2">
          ...
          >
            {setConfig.isPending ? t('accounts:youtube.integration.saving') : t('accounts:youtube.integration.save')}
          </Button>
          {saved && <span className="text-[11px] text-status-live">{t('accounts:youtube.integration.saved')}</span>}
        </div>
      )}
    </form>
  );
  ...
  return (
    <div className="rounded-lg border border-line bg-surface-sunken p-3">
      ...
      {account.status === 'reconnect_required' && (
        <p className="mt-2 text-[11px] text-status-warning">{t('accounts:youtube.account.reconnectRequiredNote')}</p>
      )}
      ...
      <ConfirmDialog
        ...
        title={t('accounts:youtube.account.disconnectDialog.title', { login: account.login })}
        message={t('accounts:youtube.account.disconnectDialog.message')}
        confirmLabel={t('accounts:youtube.account.disconnectDialog.confirm')}
        destructive
...
/**
 * YouTube panel of the Settings page's Connected Accounts section: Client ID
 * configuration, the Connect action, and the list of connected channels
 * with their health and per-account actions.
 *
 * Deliberately its own panel, separate from ConnectedAccountsPanel (Twitch)
 * - see the task's "show separate provider panels" requirement. Twitch's
 * own panel and its i18n keys are untouched by this component.
 */
export function YouTubeAccountsPanel() {
  ...
  const accounts = (accountsQuery.data ?? []).filter((a) => a.providerId === 'youtube');

  return (
    <Panel>
      <PanelHeader title={t('youtube.integration.heading')} icon={<Plug className="size-4" />} />
      <PanelBody className="space-y-4">
        <IntegrationConfigForm />

        <div className="border-t border-line pt-4">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-ink-faint">
            {t('youtube.account.heading')}
          </p>

          {!canConnect && (
            <p className="mb-2 text-[11px] text-ink-faint">{t('youtube.connect.notConfiguredNote')}</p>
          )}
          ...
          >
            {t('youtube.connect.button')}
          </Button>

          <div className="mt-3 space-y-2">
            {accounts.length === 0 ? (
              <p className="text-xs text-ink-muted">{t('youtube.account.empty')}</p>
            ) : (
      ...
      <YouTubeOAuthModal open={connecting} onClose={() => setConnecting(false)} onAuthorized={() => setConnecting(false)} />
    </Panel>
  );
}
