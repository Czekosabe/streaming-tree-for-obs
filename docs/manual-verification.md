...
| A-11 | After launch (A-2), look at the Windows notification area (system tray). | Exactly one Streaming Tree icon is present — the real logo's emblem (the neon circuit-tree mark), not the earlier placeholder (docs/windows-packaging.md §30.5). |
| A-11b | Hover the pointer over the tray icon and wait for the tooltip. | A tooltip reading "Streaming Tree for OBS" appears — this was broken (no tooltip at all) before this remediation cycle's fix (docs/windows-packaging.md §30.5c). |
...
| A-20 | Run a fresh install with the machine's Windows UI language set to English (no `/LANG=` override — let Setup pick on its own). | The Select Setup Language dialog appears, listing exactly "English" and "Polski", defaulting to English. Proceed with the default. Every subsequent wizard page and every custom dialog (Ready to Install memo, running-app cooperative-shutdown message if triggered) stays consistently in English — never a stray Polish string mixed in. |
| A-21 | Run a fresh install with the machine's Windows UI language set to Polish (no `/LANG=` override), or explicitly pick "Polski" in the Select Setup Language dialog. | Every standard Inno wizard page (Welcome, license if present, Ready to Install, Finish) renders in natural Polish, not machine-translated-sounding text. Every custom project page/message (Start Menu/desktop task checkboxes, the Ready-to-Install operation summary, any error dialog you can reasonably trigger) is also fully in Polish — no English text left mixed in. Polish diacritics (ą, ć, ę, ł, ń, ó, ś, ź, ż) render correctly everywhere, with no clipped/truncated controls or overlapping text at your normal display scale. |
| A-22 | Uninstall the A-21 (Polish) install via its Start Menu shortcut or "Apps & Features". | The uninstall confirmation dialog, the destructive-purge checkbox and its "cannot be undone" warning, and both buttons are all in Polish, matching A-21's install language — with no separate `/LANG` choice available for the uninstaller itself (there is none). The destructive checkbox's Polish wording is just as unambiguous and unchecked-by-default as the English version in A-9. |
| A-23 | Install fresh in Polish (A-21), then later run a newer build's installer over the same install location with no `/LANG=` override — once interactively, and once via the app's own "Check for updates" (tray or web) so the real silent built-in updater path is exercised. Separately, repeat with a fresh English install (A-20) updated the same two ways. | Both update paths silently keep the install's existing language — the Polish install stays Polish, the English install stays English — with no Select Setup Language dialog appearing at any point during the built-in updater's silent run. |
